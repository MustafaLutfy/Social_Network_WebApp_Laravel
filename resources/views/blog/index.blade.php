...
@section('content')
    ...
    <div class="bg-gray-50 sm:grid grid-cols-2 mx-auto py-5 px-5">
       
        <a class="fixed bottom-5 right-10 bg-[#EC5252] w-16 text-gray-100 font-bold py-4 px-5 rounded-full hover:bg-red-500 transition 0.3s" href="/blog/create"><i class="fas fa-plus text-2xl"></i></a>
  ...
  @foreach ($posts as $post)
    {{-- <div class="post-b container sm:grid grid-cols-2 gap-10 mx-auto py-20 px-5">
        ...
        <div class="flex flex-col justify-center">
            ...
            <div>
                By: <a href="/profile/{{$post->user->id}}"><span class="text-gray-500 italic "> {{$post->user->name}}</span></a>
    ...
    <main class=" w-full bg-gray-50 pb-10 flex items-center justify-center">
        <div class="border max-w-screen-md bg-white mt-6 rounded-2xl p-4">
            <div class="flex items-center justify-between">
                <div class="gap-3.5	flex items-center ">
                    <img src="/users_images/{{$post->user->image_path}}" class="object-cover bg-[#EC5252] rounded-full w-10 h-10" />
                    <div class="flex flex-col">
                        <a href="/profile/{{$post->user->id}}"><b class="mb-2 capitalize">{{$post->user->name}}</b></a>
                        <time datetime="06-08-21" class="text-gray-400 text-xs">{{Date('m-d-Y',strtotime($post->updated_at))}} at 09.15 PM
                        </time>
                    </div>
                </div>
                <div class="rounded-full h-3.5 flex	items-center justify-center">
                    <a class="text-blue-400" href="/blog/{{$post->slug}}">More Details</a>
                </div>
            </div>
            <div class="whitespace-pre-wrap mt-7">{{$post->description}}</div>
            <div class="mt-5 flex gap-2	 justify-center border-b pb-4 flex-wrap	">
                <img src="/images/{{$post->image_path}}" class="bg-red-500 rounded-2xl w-1/3 object-cover h-96 flex-auto" alt="photo">
              </div>
                <div class=" h-16 border-b  flex items-center justify-around	">
                    ...
                    <div class="pt-4">
                        <form action="/blog/{{$post->id}}/save" method="POST">
                            @csrf
                         
                        <button class="flex items-center gap-3" type="submit"> 
                ...
                <div class="flex items-center justify-between mt-4">
                    <img src="/users_images/{{Auth::user()->image_path}}"  class=" bg-[#EC5252] rounded-full w-10 h-10 object-cover border">
                    <div class="flex items-center	justify-between	 bg-gray-50 h-11 w-11/12 border rounded-2xl	 overflow-hidden px-4 ">
                        <input type="text"  class="h-full w-full bg-gray-50 border-0 outline-none " placeholder="Write your comment..." name="comment">
                    </div>
                </div>
        </div>
    </main>
